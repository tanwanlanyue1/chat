...
class Utility(private val channelName: String) {
    ...
    fun getBitmap(path: String, position: Long, maxLongSide: Long?, result: MethodChannel.Result): Bitmap {
        ...
        val max = Math.max(width, height) * 1f
        if (maxLongSide != null && max > maxLongSide) {
            val scale = maxLongSide / max
