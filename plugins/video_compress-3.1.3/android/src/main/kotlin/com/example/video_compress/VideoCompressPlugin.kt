    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            "getByteThumbnail" -> {
                ...
                val longSide = call.argument<Int>("longSide") // to long
                ThumbnailUtility(channelName).getByteThumbnail(path!!, quality, position.toLong(), longSide?.toLong(), result)
            }
            "getFileThumbnail" -> {
                val path = call.argument<String>("path")
                val quality = call.argument<Int>("quality")!!
                ...
                ThumbnailUtility("video_compress").getFileThumbnail(context, path!!, quality,
                        position.toLong(), longSide?.toLong(), result)
            }
            "getMediaInfo" -> {
                val path = call.argument<String>("path")
